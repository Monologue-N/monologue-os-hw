...
kthread_create(struct proc *p, kthread_func_t func, long arg1, void *arg2)
{
        ...
        KASSERT(t != NULL && "No enough memory for kthread_t");

        /* set up this thread's context */
        char* kstack = (char*) alloc_stack();
        KASSERT(kstack != NULL && "No enough memory for stack");
        t->kt_kstack = kstack;

        /* DEFAULT_STACK_SIZE is from config.h, and should have explicit typecasting for arg1 */
        // not sure, maybe: PAGE_SIZE * (1 + (DEFAULT_STACK_SIZE >> PAGE_SHIFT))
        context_setup(&t->kt_ctx, func, (int)arg1, arg2, t->kt_kstack, DEFAULT_STACK_SIZE, p->p_pagedir);

        /* set this threads' state */
        t->kt_state = KT_RUN;  // KT_RUN is from kthreads.h

        /* set the thread's process */
        t->kt_proc = p;

        /* set thread to "hasn't been cancelled", which is 0 */
        t->kt_cancelled = 0;

        /* set the queue that this thread is blocked on */
        t->kt_wchan = NULL;

        /* ---------- the thread's link on a queue --------- */
 
        /* initialize link on ktqueue */
        list_link_init(&t->kt_qlink);

        /* initialize link on proc thread list, p_threads */
        list_link_init(&t->kt_plink);

        /* connect plink with p_threads list */
...
kthread_cancel(kthread_t *kthr, void *retval)
{
        ...
        /* if thread to be cancelled is current, just call kthread_exit */
        if (kthr == curthr){
                kthread_exit(retval);
                /* never return */
        }
        /* set the cancelled and retval fields of the thread */
        kthr->kt_cancelled = 1;
...
kthread_exit(void *retval)
{
        ...
        /* set the thread's retval field */
        curthr->kt_retval = retval;

        /* set the thread's state */
        curthr->kt_state = KT_EXITED; // KT_EXITED is from kthread.h
