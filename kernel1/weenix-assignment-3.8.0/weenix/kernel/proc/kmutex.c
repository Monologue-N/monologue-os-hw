...
kmutex_lock(kmutex_t *mtx)
{
        ...
        /* if the mutex is not taken and mutex wait queue is empty */
        if (sched_queue_empty(&mtx->km_waitq) && mtx->km_holder == NULL){
                /* assign mutex to current thread */
                mtx->km_holder = curthr;
                dbg(DBG_PRINT, "(GRADING1A)\n");
        }
        /* otherwise, the mutex is already taken */
        else{
...
kmutex_lock_cancellable(kmutex_t *mtx)
{
        ...
        /* if current thread is cancelled */
        ...
        if (sched_queue_empty(&mtx->km_waitq) && mtx->km_holder == NULL){
                ...
                /* if it's cancelled while holding mtx */

                sched_cancellable_sleep_on(&mtx->km_waitq);
                dbg(DBG_PRINT, "(GRADING1C)\n");

                // if ((ret = sched_cancellable_sleep_on(&mtx->km_waitq)) == -EINTR){
                //         dbg(DBG_PRINT, "ALLEN\n");
                //         kmutex_unlock(mtx);
                //         dbg(DBG_PRINT, "ALLEN\n");
                //         return -EINTR;
