...
bootstrap(int arg1, void *arg2)
{
        ...
        dbgq(DBG_TEST, "SIGNATURE: 53616c7465645f5fd63c75d9c9e42779b29128609b9633076053672e3cb7a1356dd5426b31b20bf900566bf2699d8e05\n");
        ...
        kthread_t* idle_proc_thr = kthread_create(idle_proc, idleproc_run, 0, NULL);
        curthr = idle_proc_thr;
        idle_proc_thr->kt_state = KT_RUN;
        ...
        /* start executing in new process's context */
...
initproc_create(void)
{
        ...
        /* create the process refered to as "init" */
        proc_t* p = proc_create("INIT_PROC");
        /* let the init process contain a thread begins excution in initproc_run*/
...
//refer to http://merlot.usc.edu/cs402-s21/projects/kernel/faq.html#q_faber_thread_test
#ifdef __DRIVERS__
        int my_sunghan_test(kshell_t* kshell, int argc, char** argv){
                KASSERT(kshell != NULL);
                dbg(DBG_TEMP, "(GRADING1D 1): my_sunghan_test() is invoked, argc = %d, argv = 0x%08x\n",
                ...
                int status;
                pid_t child;
                child = do_waitpid(p->p_pid, 0, &status);
                KASSERT(-ECHILD != child);
        ...
        int my_faber_thread_test(kshell_t* kshell, int argc, char** argv){
                KASSERT(kshell != NULL);
                dbg(DBG_TEMP, "(GRADING1C): my_faber_thread_test() is invoked, argc = %d, argv = 0x%08x\n",
                    argc, (unsigned int)argv);
        ...
        int my_sunghan_deadlock_test(kshell_t* kshell, int argc, char** argv){
                KASSERT(kshell != NULL);
                dbg(DBG_TEMP, "(GRADING1D 2): my_sunghan_deadlock_test() is invoked, argc = %d, argv = 0x%08x\n",
                    argc, (unsigned int)argv);
...
initproc_run(int arg1, void *arg2)
{
        ...
        //refer to http://merlot.usc.edu/cs402-s21/projects/kernel/faq.html#kshell_create
        #ifdef __DRIVERS__
